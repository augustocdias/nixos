{jailed ? false}: let
  # Builds go through the daemon, which sandboxes them, and `trusted-users` is
  # root-only — a client cannot turn that sandbox off or reach the system
  # profile. run/shell/develop execute fetched code unsandboxed as the user
  # and plain `opencode` is still unjailed, so those keep a prompt.
  #
  # The "*<cmd>*" variants are deliberately absent when jailed: a prefixed form
  # (`sudo nix build`, `env nix build`) then falls through to "*" = "ask"
  ...
  baseBash = {
    ...
    # Never permitted, jailed or not: activates a system, mutates a profile,
    # or collects garbage as root via the daemon.
    "nix profile*" = "deny";
...
in rec {
  readOnlyBash = baseBash // nixCommands;

...
- **Voice stack**: `launchd.daemons` for `ollama` (`OLLAMA_HOST=[::]:11434` — the Pi resolves `macmini.local` to an IPv6 ULA first), `wyoming-faster-whisper` (port 10300, `--language auto` for EN+pt-BR; needs `HF_HOME=/tmp`, an upstream bug) and `wyoming-piper` (port 10200). `services.wyoming.*` and `services.ollama` are NixOS-only, hence hand-rolled by the `voiceDaemon` helper. State dirs are created in `postActivation`.
  ...
  - **Untested: the macOS Application Firewall.** It is enabled, and Nix binaries are ad-hoc/linker-signed, so they qualify for neither `allowSigned` nor `allowSignedApp`. Loopback is ALF-exempt and `socketfilterfw --getappblocked` answers "permitted" even for nonexistent paths, so neither proves anything — only a connection from the Pi does. If it turns out to block, nix-darwin's `networking.applicationFirewall` exposes global toggles only (`enable`, `blockAllIncoming`, `allowSigned`, `allowSignedApp`, `enableStealthMode`); a per-app allowlist needs an activation script calling `socketfilterfw --add`, which accumulates stale entries as store paths churn.
